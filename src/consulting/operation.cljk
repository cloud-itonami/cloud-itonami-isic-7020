(ns consulting.operation
  "OperationActor -- one management-consultancy operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Consultant-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Consulting Engagement Governor
  ...
  One graph run = one management-consultancy operation (intake ->
  ...
  decision to a human operator (the consultant). The approver resumes
  with `{:approval {:status :approved}}` (or :rejected). `:actuation/
  issue-deliverable` ALWAYS reaches this node when the governor is
  clean -- see `consulting.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [consulting.consultantadvisor :as consultantadvisor]
            [consulting.governor :as governor]
            [consulting.phase :as phase]
...
(defn build
  ...
  `consulting.store/Store`).
  opts:
    :advisor      -- a `consulting.consultantadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (consultantadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Consultant-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (consultantadvisor/-advise advisor store request)]
            {:proposal p :audit [(consultantadvisor/trace request p)]})))

      ;; Consulting Engagement Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
