(ns consulting.phase-test
  "The phase table as executable tests. The invariant this repo cannot
  regress on: `:actuation/issue-deliverable` must NEVER be a member of
  any phase's `:auto` set."
  (:require [clojure.test :refer [deftest is testing]]
            [consulting.phase :as phase]))

(deftest issue-deliverable-never-auto-at-any-phase
  (testing "structural invariant: no phase, now or in the future entries, auto-commits a real deliverable issuance"
    (doseq [[n {:keys [auto]}] phase/phases]
      (is (not (contains? auto :actuation/issue-deliverable))
          (str "phase " n " must not auto-commit :actuation/issue-deliverable")))))
...
(deftest gate-escalates-a-clean-non-auto-write
  (is (= :escalate (:disposition (phase/gate 3 {:op :actuation/issue-deliverable} :commit)))))
